    ...
    if test_connection:
        try:
            ...
            if response.status_code == 404:
                return False, f"Image deployment '{AZURE_OPENAI_IMAGE_DEPLOYMENT}' not found"
...
def check_chat_deployment(test_connection: bool = False) -> Tuple[bool, str]:
    ...
    if test_connection:
        try:
            ...
            if response.status_code == 404:
                return False, f"Chat deployment '{AZURE_OPENAI_CHAT_DEPLOYMENT}' not found"
...
def check_tts_deployment(test_connection: bool = False) -> Tuple[bool, str]:
    ...
    if test_connection:
        try:
            ...
            if response.status_code == 404:
                return False, f"TTS deployment '{AZURE_OPENAI_TTS_DEPLOYMENT}' not found"
